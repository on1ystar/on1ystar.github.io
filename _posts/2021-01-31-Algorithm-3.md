...
에를 들어 효율성 테스트 제한 사항에 따라 [1, 2, 3, 4, 5, ... , 200000]만 되도 `iter_cnt`는 계속해서 1로 밖에 초기화되지 않기 때문에(최초에 1이고, 모든 원소에서 1씩 빼기 때문에 다시 최소값이 1이 됨) 총 200000 * 200000 = 4*10^10 = 400억 번의 반복을 해야 한다.
